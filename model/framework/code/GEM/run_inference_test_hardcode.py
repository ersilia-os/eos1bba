...
def main(args):
    """tbd"""
    compound_encoder_config = load_json_config('/Users/karthik/eos1bba/model/framework/code/GEM/model_configs/geognn_l8.json')  #this line encodes the configurations from the model. Change this to include the direct file path
    model_config = load_json_config('/Users/karthik/eos1bba/model/framework/code/GEM/model_configs/pretrain_gem.json')  #change this to include the direct file path. Then I can change parameters of the function as well. 
    ...
    model.set_state_dict(paddle.load('/Users/karthik/eos1bba/model/framework/code/GEM/pretrain_models-chemrl_gem/class.pdparams'))
    if args.distributed:
    ...
    #datgit aset = load_smiles_to_dataset(args.data_path)
    # if args.DEBUG:
    ...
    transform_fn = GeoPredTransformFn(model_config['pretrain_tasks'], model_config['mask_ratio'])  # the GeoPred transform function and the collate function uses MMFF 3D optimization. 
    ...
    #get embedding–layer (the generic one) before predictions. 
